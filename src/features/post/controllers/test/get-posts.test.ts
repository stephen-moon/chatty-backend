...
import { postMockData, postMockRequest, postMockResponse } from '@root/mocks/post.mock';
...
describe('Get', () => {
  ...
  describe('posts', () => {
    it('should send correct json response if posts exist in cache', async () => {
      const req: Request = postMockRequest({}, authUserPayload, { page: '1' }) as Request;
    ...
    it('should send empty posts if any post does not exist in cache and database', async () => {
      const req: Request = postMockRequest({}, authUserPayload, { page: '1' }) as Request;
      const res: Response = postMockResponse();
      jest.spyOn(PostCache.prototype, 'getPostsFromCache').mockResolvedValue([]);
      jest.spyOn(PostCache.prototype, 'getTotalPostsInCache').mockResolvedValue(0);
